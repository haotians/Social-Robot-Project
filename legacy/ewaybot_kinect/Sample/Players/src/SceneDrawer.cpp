...
void write_joint_file(XnUserID player, XnSkeletonJoint eJoint1, FILE *file_id1)
{
if (!g_UserGenerator.GetSkeletonCap().IsCalibrated(player))
	{
		printf("not calibrated!\n");
	...
	fprintf(file_id1,"%.4f %.4f %.4f ", pt[0].X, pt[0].Y, pt[0].Z);
	// printf("Limb (%.4f, %.4f)\n", pt[0].X, pt[0].Y);
}
...
//		printf("Initialized depth texture: width = %d, height = %d\n", texWidth, texHeight);
	...
	if (player != 0)
	{
		...
		write_joint_file(player, XN_SKEL_LEFT_HAND,file_id);
		write_joint_file(player, XN_SKEL_LEFT_ELBOW,file_id);
		write_joint_file(player, XN_SKEL_LEFT_SHOULDER,file_id);

		write_joint_file(player, XN_SKEL_LEFT_SHOULDER,file_id);
		write_joint_file(player, XN_SKEL_NECK,file_id);
		write_joint_file(player, XN_SKEL_RIGHT_SHOULDER,file_id);


		write_joint_file(player, XN_SKEL_RIGHT_SHOULDER,file_id);
		write_joint_file(player, XN_SKEL_RIGHT_ELBOW,file_id);
		write_joint_file(player, XN_SKEL_RIGHT_HAND,file_id);
