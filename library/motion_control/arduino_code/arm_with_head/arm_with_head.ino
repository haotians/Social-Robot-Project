...
int neck_pwm = 5;
int servo_position[SERVO_NUM] = {1500, 1500};
int neck_position = 0;
...
void setup()
{
  ...
  // initialize servos
  head_servo[0].attach(servo_pin[0]);
  head_servo[1].attach(servo_pin[1]);
...
void move_head()
{
  head_servo[0].writeMicroseconds(servo_position[0]);
  head_servo[1].writeMicroseconds(servo_position[1]);
  if (neck_position == 0){
...
{
  int temp_trash = 0;
  if(Serial.available() >= (1 + 4 * SERVO_NUM + 2 * 3)){
    ...
    servo_position[0] = Serial.read();
    servo_position[0] = servo_position[0] + (Serial.read() << 8);
    servo_position[1] = Serial.read();
    servo_position[1] = servo_position[1] + (Serial.read() << 8);
