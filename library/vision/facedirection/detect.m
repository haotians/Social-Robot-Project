...
pyra = featpyramid(input,model,layer);
[components,filters_temp,resp]  = modelcomponents(model,pyra);
num_casade = length(filters_temp);
filters = cell(num_casade,1);

for i = 1:num_casade;
    filters{i} = filters_temp{i}(:,:,layer(1):layer(2));
end
...
for c  = [1,4,10,13],
    for rlevel = [1],
        ...
        for k = 1:numparts,
            f     = parts(k).filterid;
            if isempty(resp{rlevel}),
                resp{rlevel} = fconv(pyra.feat{rlevel},filters,1,length(filters));
            end
            parts(k).score = resp{rlevel}{f};
            parts(k).level = rlevel;
