...
int run_asr(UserData *udata, const char *asr_audiof, const char *rec_rslt);
...
int run_asr(UserData *udata, const char *asr_audiof){
    char *rec_rslt = NULL;
    return run_asr(udata, asr_audiof, rec_rslt);
}
...
int run_asr(UserData *udata, const char *asr_audiof, const char *rec_rslt)
{
    ...
    // time(&later);
    // seconds = 1000*difftime(later, now);
    gettimeofday(&t2, NULL);
...
{
    const char *login_config    = "appid = 565080c2"; //登录参数
    UserData asr_data; 
    int ret                     = 0 ;
    char c;

    char* file_audio = "test.pcm";
